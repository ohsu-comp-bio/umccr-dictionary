...
class Gen3Configuration(object):
    ...
    def properties(self, template, schema, parent=None, black_list=None, white_list=[], parent_description=None, type_offset=0, is_root=False):
        ...
        for el in schema['snapshot']['element']:
            ...
            if name in ['type']:
                continue
            # logger.debug(f"{name} {el['id']}")

            if '[x]' in name:
                # original_name = name
                name = name.replace('[x]', '')
                name = name.split(':')[-1]
                # logger.debug(f"multivalued field {original_name} {name}")
                    ...
                    for white_list_name_part in white_list_name.split('.'):
                        ...
                        if parent == white_list_name_part \
                            and full_name == parent:
                                skip = False
                if skip:
                    # logger.debug(f">>> white_list skipping parent:{parent} name_base:{name_base} name:{name} full_name:{full_name}")
                    continue

            # logger.debug(f"Working on: {name} {el['id']}")
                    ...
                    if 'WARNING' in property_attributes.get('enum', ''):
                        ...
                        if template['properties'][name].get('enum', None) == 'WARNING_MISSING_ENUM':
                            if 'enums' in self._config[base] and name in self._config[base]['enums']:
                                ...
                                logger.warning(f"WARNING_MISSING_ENUM xxx {base} {type} {name}")
            ...
            def expand_types(type):
                ...
                if 'patternCodeableConcept' in template['properties'][name]:
                    logger.debug(f"setting patternCodeableConcept for {full_name} in profile_template")
                    profile_template['patternCodeableConcept'] = template['properties'][name]['patternCodeableConcept']
            ...
            if 'multiple_types' in property_attributes:                
                for type in property_attributes['multiple_types']:
                    ...
                    assert profile_schema, f"No schema found for {type}"
    ...
    def normalize(self, template, base):
        ...
        for p in template['properties']:
            ...
            # # logger.debug(f"patternCodeableConcept??? {p} {list(template['properties'][p].keys())}")
            # logger.debug(f"patternCodeableConcept??? {p} {list(self._parent_element.keys())}")
            
            logger.debug(f"??? {p} {list(template['properties'][p].keys())}")
            if 'patternCodeableConcept' in template['properties'][p]:
                logger.debug(f"patternCodeableConcept ??? {template['properties'][p]['patternCodeableConcept']}")
        ...
        for p in template['properties']:
            ...
            if template['properties'][p].get('enum', None) == 'WARNING_MISSING_ENUM':
                if 'enums' in self._config[base] and p in self._config[base]['enums']:
                    ...
                    logger.warning(f"WARNING_MISSING_ENUM normalize {base} {type} {p} {template['properties'][p]}")
